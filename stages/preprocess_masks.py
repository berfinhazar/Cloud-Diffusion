...
def process_dataset(lcib_dir, output_dir, threshold=THRESHOLD):
    ...
    sample_dirs = sorted([d for d in lcib_dir.iterdir() if d.is_dir()])
    print(f"Toplam {len(sample_dirs)} klasör bulundu.")

    total_masks   = 0
    total_shadows = 0

    for sample_dir in sample_dirs:
        # Çıktı klasörünü oluştur (aynı isimde)
        out_sample = output_dir / sample_dir.name
        out_sample.mkdir(parents=True, exist_ok=True)

        # Tüm mask ve shadow PNG'lerini bul
